...
int main(void)
{
   ...
   RUN_TEST(test_VectorConcatenate_BasicAppend_Ints);
//   RUN_TEST(test_VectorConcatenate_AppendLargeToSmall);
//   RUN_TEST(test_VectorConcatenate_EmptyAppend);
//   RUN_TEST(test_VectorConcatenate_AppendToEmpty);
//   RUN_TEST(test_VectorConcatenate_OffByOne_LastElementPreserved);
//   RUN_TEST(test_VectorConcatenate_InsufficientCapacity);
...
void test_VectorConcatenate_BasicAppend_Ints(void)
{
   struct Vector_S *vec1 = VectorInit(sizeof(int), 5, 10, 0);
   struct Vector_S *vec2 = VectorInit(sizeof(int), 5, 10, 0);

   int vals1[] = {1, 2, 3};
   int vals2[] = {4, 5};

   for (size_t i = 0; i < 3; i++) VectorPush(vec1, &vals1[i]);
   for (size_t i = 0; i < 2; i++) VectorPush(vec2, &vals2[i]);

   TEST_ASSERT_TRUE(VectorConcatenate(vec1, vec2));
   TEST_ASSERT_EQUAL_UINT32(5, VectorLength(vec1));
   TEST_ASSERT_EQUAL_INT(1, *(int *)VectorGetElementAt(vec1, 0));
   TEST_ASSERT_EQUAL_INT(2, *(int *)VectorGetElementAt(vec1, 1));
   TEST_ASSERT_EQUAL_INT(3, *(int *)VectorGetElementAt(vec1, 2));
   TEST_ASSERT_EQUAL_INT(4, *(int *)VectorGetElementAt(vec1, 3));
   TEST_ASSERT_EQUAL_INT(5, *(int *)VectorGetElementAt(vec1, 4));
...
void test_VectorConcatenate_AppendLargeToSmall(void)
{
   #define LARGE_VEC_LEN 10000000
   struct Vector_S * vec1 = VectorInit( sizeof(int),
                                        5,
                                        10,
                                        0 );
   struct Vector_S * vec2 = VectorInit( sizeof(int),
                                       LARGE_VEC_LEN,
                                       LARGE_VEC_LEN * 10,
                                       0 );
   ...
   for (size_t i = 0; i < LARGE_VEC_LEN; i++) VectorPush(vec2, 0);

   TEST_ASSERT_TRUE(VectorConcatenate(vec1, vec2));
   TEST_ASSERT_GREATER_OR_EQUAL_size_t( LARGE_VEC_LEN + 5, VectorLength(vec1) );
   for ( size_t i = 3; i < LARGE_VEC_LEN; i++ )
   {
      TEST_ASSERT_EQUAL_INT( 0, *((int *)VectorGetElementAt(vec1, i)) );
   }
...
void test_VectorConcatenate_EmptyAppend(void)
{
   ...
   // Append empty vector
   TEST_ASSERT_TRUE(VectorConcatenate(vec1, vec2));
   TEST_ASSERT_EQUAL_UINT32(3, VectorLength(vec1));
   for (size_t i = 0; i < 3; i++)
      TEST_ASSERT_EQUAL_INT(vals1[i], *(int *)VectorGetElementAt(vec1, i));
...
void test_VectorConcatenate_AppendToEmpty(void)
{
   ...
   TEST_ASSERT_TRUE(VectorConcatenate(vec1, vec2));
   TEST_ASSERT_EQUAL_UINT32(2, VectorLength(vec1));
   TEST_ASSERT_EQUAL_INT(4, *(int *)VectorGetElementAt(vec1, 0));
   TEST_ASSERT_EQUAL_INT(5, *(int *)VectorGetElementAt(vec1, 1));
...
void test_VectorConcatenate_OffByOne_LastElementPreserved(void)
{
   struct Vector_S *vec1 = VectorInit(sizeof(int), 3, 10, 0);
   struct Vector_S *vec2 = VectorInit(sizeof(int), 3, 10, 0);
   ...
   // Save last element before concatenation
   int last_before = *(int *)VectorGetElementAt(vec1, VectorLength(vec1) - 1);

   TEST_ASSERT_TRUE(VectorConcatenate(vec1, vec2));
   // Ensure last element of vec1 before concat is still present at correct index
   TEST_ASSERT_EQUAL_INT(last_before, *(int *)VectorGetElementAt(vec1, 2));
   // Ensure appended elements are at correct indices
   TEST_ASSERT_EQUAL_INT(40, *(int *)VectorGetElementAt(vec1, 3));
   TEST_ASSERT_EQUAL_INT(50, *(int *)VectorGetElementAt(vec1, 4));
...
void test_VectorConcatenate_InsufficientCapacity(void)
{
   struct Vector_S *vec1 = VectorInit(sizeof(int), 2, 4, 0);
   struct Vector_S *vec2 = VectorInit(sizeof(int), 3, 3, 0);

   int vals1[] = {1, 2};
   int vals2[] = {3, 4, 5};

   for (size_t i = 0; i < 2; i++) VectorPush(vec1, &vals1[i]);
   for (size_t i = 0; i < 3; i++) VectorPush(vec2, &vals2[i]);

   // This should fail because max_capacity is 4 and total would be 5
   TEST_ASSERT_FALSE(VectorConcatenate(vec1, vec2));
...
void test_VectorConcatenate_NullArguments(void)
{
   ...
   TEST_ASSERT_FALSE(VectorConcatenate(NULL, vec));
   TEST_ASSERT_FALSE(VectorConcatenate(vec, NULL));
   TEST_ASSERT_FALSE(VectorConcatenate(NULL, NULL));
